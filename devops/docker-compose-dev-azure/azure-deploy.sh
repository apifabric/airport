...
resourcename="airport"  # lower case, only
resourcegroup="airport_rg"
...
# git clone https://github.com/apilogicserver/airport.git
# cd airport
...
az group create --name airport_rg --location "westus"
az appservice plan create --name myAppServicePlan --resource-group airport_rg --sku S1 --is-linux
az container create --resource-group airport_rg --name airport --image apilogicserver/airport:latest --dns-name-label airport --ports 5656 --environment-variables 'VERBOSE'='True'  'APILOGICPROJECT_CLIENT_URI'='//airport.westus.azurecontainer.io:5656'
